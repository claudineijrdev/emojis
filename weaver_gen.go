...
func init() {
	...
	codegen.Register(codegen.Registration{
		...
		LocalStubFn: func(impl any, caller string, tracer trace.Tracer) any {
			return searcher_local_stub{impl: impl.(Searcher), tracer: tracer, searcherMetrics: codegen.MethodMetricsFor(codegen.MethodLabels{Caller: caller, Component: "emojis/Searcher", Method: "Searcher", Remote: false, Generated: true})}
		},
		ClientStubFn: func(stub codegen.Stub, caller string) any {
			return searcher_client_stub{stub: stub, searcherMetrics: codegen.MethodMetricsFor(codegen.MethodLabels{Caller: caller, Component: "emojis/Searcher", Method: "Searcher", Remote: true, Generated: true})}
		},
		ServerStubFn: func(impl any, addLoad func(uint64, float64)) codegen.Server {
...
type searcher_local_stub struct {
	impl            Searcher
	tracer          trace.Tracer
	searcherMetrics *codegen.MethodMetrics
}

// Check that searcher_local_stub implements the Searcher interface.
var _ Searcher = (*searcher_local_stub)(nil)

func (s searcher_local_stub) Searcher(ctx context.Context, a0 string) (r0 []string, err error) {
	// Update metrics.
	begin := s.searcherMetrics.Begin()
	defer func() { s.searcherMetrics.End(begin, err != nil, 0, 0) }()
	span := trace.SpanFromContext(ctx)
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
		ctx, span = s.tracer.Start(ctx, "main.Searcher.Searcher", trace.WithSpanKind(trace.SpanKindInternal))
		defer func() {
			if err != nil {
				span.RecordError(err)
				span.SetStatus(codes.Error, err.Error())
...
func (s searcher_client_stub) Searcher(ctx context.Context, a0 string) (r0 []string, err error) {
	// Update metrics.
	var requestBytes, replyBytes int
	begin := s.searcherMetrics.Begin()
	defer func() { s.searcherMetrics.End(begin, err != nil, requestBytes, replyBytes) }()

	span := trace.SpanFromContext(ctx)
	if span.SpanContext().IsValid() {
		// Create a child span for this method.
		ctx, span = s.stub.Tracer().Start(ctx, "main.Searcher.Searcher", trace.WithSpanKind(trace.SpanKindClient))
	}
...
func (s searcher_server_stub) searcher(ctx context.Context, args []byte) (res []byte, err error) {
	// Catch and return any panics detected during encoding/decoding/rpc.
	defer func() {
		if err == nil {
			err = codegen.CatchPanics(recover())
	...
	// user code: fix this.
	// Call the local method.
	r0, appErr := s.impl.Searcher(ctx, a0)
...
func (s searcher_reflect_stub) Searcher(ctx context.Context, a0 string) (r0 []string, err error) {
	err = s.caller("Searcher", ctx, []any{a0}, []any{&r0})
	return
}
